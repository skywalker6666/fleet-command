...
- [~] 高階點數 / 哩程估值再細化，已補上首批航空計畫 profile-aware 命中，持續補齊更多銀行別與航空計畫 program-level 估值
...
- 更細的 program-level explainability 仍是後續項目，但 API 已會依 promotion metadata 命中首批航空計畫 row。
...
- 已補基礎結果 explainability（換算式 / source / note），API 端也已能依 `cardCode` / `cardName` / `title` / `conditions` 命中 `ASIA_MILES`、`EVA_INFINITY` 等 miles profile；但尚未對高階哩程玩家提供更完整的 program-level explainability
...
1. 補強 `POINTS` / `MILES` 的更多銀行別與 program-level 估值，目前先完成首批 miles profile-aware 命中
